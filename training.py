...
def gradient_descent_algorithm(
    data, learningRate=0.1, max_iterations=1000, threshold=0.0001
):
...
def main():
    ...
    theta0, theta1 = gradient_descent_algorithm(
        normalized_data, 0.1, 10000, 0.00001
    )

    # STEP 4 : denormalize thetas
    # formula to get back real thetas :
    # value = value_normalized * (max - min) + min
    real_theta1 = (
        theta1 * (max_price - min_price) / (max_mileage - min_mileage)
    )
    real_theta0 = (
        min_price + theta0 * (max_price - min_price)
        - real_theta1 * min_mileage
    )

    print(
        f"\nFinal thetas : theta0={real_theta0:.2f}, "
        f"theta1={real_theta1:.6f}"
    )
